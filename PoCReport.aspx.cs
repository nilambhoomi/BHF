...
public partial class PoCReport : System.Web.UI.Page
{
    ...
    {
        string query = "select  tp.ProcedureDetail_ID,pm.sex,pm.LastName+', '+pm.FirstName 'Name',ie.Compensation as 'Case' ,(select top 1 ISNULL(CONVERT(VARCHAR(10),A.DOE,101),'') from tblFUPatient A where A.PatientIE_ID=ie.PatientIE_ID order by A.DOE DESC) 'DOE',ISNULL(CONVERT(VARCHAR(10),pm.DOB,101),'') as DOB ,ISNULL(CONVERT(VARCHAR(10),ie.DOA,101),'') as DOA ,tp.MCODE,ISnull(pm.MC,'') AS MC,pm.phone+','+pm.Phone2 as Phone,lc.location,(select ic.InsCo from tblInsCos ic where ic.InsCo_ID=ie.InsCo_ID) 'Ins Co',ie.claimnumber 'Claim Number',pm.policy_no 'Policy No',a.Attorney,case when pm.Vaccinated = 1 then 'YES' when pm.Vaccinated = 0 then 'NO' else 'NO' end as Vaccinated ";
        string condition = " from  tblProceduresDetail tp  inner join tblProcedures pp on pp.MCODE = tp.MCODE and tp.BodyPart= pp.BodyPart inner join tblPatientIE ie on tp.PatientIE_ID = ie.PatientIE_ID inner join tblPatientMaster pm on pm.Patient_ID=ie.Patient_ID left join dbo.tblLocations lc ON ie.Location_ID = lc.Location_ID inner join tblAttorneys a on a.Attorney_ID = ie.Attorney_ID ";
        ...
        if (!string.IsNullOrEmpty(condition1))
        {
         ...
         if (Convert.ToInt32(ddl_location.SelectedValue) > 0)
        {
            query += " and lc.Location_ID = isnull(" + ddl_location.SelectedValue + ",lc.Location_ID)";
        }
    ...
    public string SortExpression
    {
        get
        {
            ...
            {
                ViewState["z_sortexpresion"] = gvProcedureTbl.DataKeyNames[0].ToString();
            }
    ...
    protected void UpdateDetails(object sender, EventArgs e)
    {
        ...
        ClientScript.RegisterStartupScript(GetType(), "alert", "alert('Id: " + ProcedureDetail_ID + " Name: " + name + " reqdate: " + reqdate + "')", true);
    ...
    protected void btnReshedule_Click(object sender, EventArgs e)
    {
        ...
        hidID.Value = btn.CommandArgument;
        hidType.Value = "1";
        // Reschedules(btn.CommandArgument);
        ClientScript.RegisterStartupScript(GetType(), "PopupIE", "opensignupload();", true);
    }
    ...
    public void Reschedules(string id)
    {
        ...
        DBHelperClass dBHelper = new DBHelperClass();
        string query = "update tblProceduresDetail set Scheduled='" + txtSDate.Text + "' where ProcedureDetail_ID=" + id;
        dBHelper.executeQuery(query);
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        if (hidType.Value == "1")
        {
            string query = "update tblProceduresDetail set Scheduled='" + txtSDate.Text + "' where ProcedureDetail_ID=" + hidID.Value;
            dBHelper.executeQuery(query);
        }
        else if (hidType.Value == "0")
        {
            foreach (GridViewRow grow in gvProcedureTbl.Rows)
            {
                //Searching CheckBox("chkDel") in an individual row of Grid  
