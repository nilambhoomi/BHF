...
public partial class Page1 : System.Web.UI.Page
{
    ...
    public static string[] getInsComp(string prefix)
    {
        ...
        DataSet ds = db.selectData("select InsCo_ID,InsCo,(Address1) as fullAddress,Telephone  from tblInsCos where InsCo like '%" + prefix + "%'");
        if (ds.Tables[0].Rows.Count > 0)
        {
            string cmpname = "",address="",telPhone="";
            for (int i = 0; i <= ds.Tables[0].Rows.Count - 1; i++)
            {
                cmpname = ds.Tables[0].Rows[i]["InsCo"].ToString();
                address = ds.Tables[0].Rows[i]["fullAddress"].ToString();
                telPhone = ds.Tables[0].Rows[i]["Telephone"].ToString();
                inscmp.Add(string.Format("{0}-{1}-{2}-{3}", cmpname, ds.Tables[0].Rows[i]["InsCo_ID"].ToString(),address,telPhone));
            }
        }
    ...
    private void bindEditData(string PatientIEid)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            txt_ins_address.Text = ds.Tables[0].Rows[0]["InsAddress1"].ToString();
            txt_ins_phone.Text = ds.Tables[0].Rows[0]["InsPhone"].ToString();
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        param[37] = new SqlParameter("@EmpPhone", txtEmpPhone.Text.Trim());
        param[38] = new SqlParameter("@EmpFax", txtEmpFax.Text.Trim());
        ...
        param[42] = new SqlParameter("@InsAddress", txt_ins_address.Text);
        param[43] = new SqlParameter("@InsPhone", txt_ins_phone.Text);
        ...
        if (!string.IsNullOrEmpty(patientIE_ID))
        {
            ...
            param[45] = new SqlParameter("@PatientIE_ID", Session["PatientIE_ID"]);
        }
